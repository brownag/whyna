...
propagates naturally, and you must explicitly handle it with
[`is.na()`](https://rdrr.io/r/base/NA.html) or `na.rm = TRUE`. This
forces you to confront missing data rather than accidentally corrupting
your analysis.
...
**`informative_na(x, type = NULL)`** Alias for
[`why_na()`](http://humus.rocks/whyna/reference/new_informative_na.md)
(verbose form, identical behavior).
...
[`rbind()`](https://rdrr.io/r/base/cbind.html)** Combines with plain
numeric vectors, upgrading to `informative_na`.
...
- **Type preservation**: Input type is preserved in
  [`why_na()`](http://humus.rocks/whyna/reference/new_informative_na.md).
  Use the optional `ptype` argument for explicit casting.
- **vctrs foundation**: Built on
  [`vctrs::new_rcrd()`](https://vctrs.r-lib.org/reference/new_rcrd.html)
  for integration with tidyverse (dplyr, tibble).
